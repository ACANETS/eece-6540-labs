...
source "$script_dir/emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_report_timing_core.tcl"


if [ info exists ddr_db ] {
   unset ddr_db
}
emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_corename}"
   } else {
      set fbasename "${::GLOBAL_emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_corename}_${inst_id}"
   }
   ...
   emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_perform_read_capture_analysis $opcname $inst pins var summary
   
   #######################################
   # Write Analysis

   emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis 
   
   emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # PHY Analyses
   
   emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_sort_proc $summary]
   ...
   puts $f "Core: ${::GLOBAL_emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_corename} - Instance: $inst"
   puts $f "Path, Setup Margin, Hold Margin"

   
   post_message -type info "Core: ${::GLOBAL_emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ emif_ddr4_ddr4a_altera_emif_arch_nf_191_eh64ybq_round_3dp $hold]
      }
