...
proc set_jtag_timing_constraints { } {
    # If the timing characteristic outside of FPGA is well understood, and there is a need to provide 
    # more slack to allow flexible placement of JTAG logic in the FPGA core, use the timing constraints
    # for both timing analysis and fitter; otherwise, use the default fitter timing constraints.
    ...
    if { $use_fitter_specific_constraint && [string equal quartus_fit $::TimeQuestInfo(nameofexecutable)] } {
        # Define a different set of timing spec to influence place-and-route result in the jtag clock domain.
        # The slacks outside of FPGA are maximized.
        
        set_default_quartus_fit_timing_directive
    }  else {
        # Define a set of timing constraints that describe the JTAG paths for TimeQuest to analyze. 
        # TimeQuest timing reports show whether the JTAG logic in FPGA core will operates in this setup.
...
proc set_default_quartus_fit_timing_directive { } {
    # Arria10 supports max 33.3Mhz clock.  This clock rate is specified based on the JTAG IO boundary scan performance.
    ...
    # Force fitter to place register driving TDO pin to be as close to JTAG controller as possible
    # to maximize the slack outside of FPGA.  
    set_max_delay -to [get_ports { altera_reserved_tdo } ] 0
    
    # tdi and tms are not constrained.  In most design, the circuit produced by Quartus II compiler works as
    # they are skew matched with tck mostly before reaching FPGA.  Half tck cycle slack allows much placement and
    # route freedom to meet timing. There is no usable alternative without either over-constraining or using 
    # the proper timing constraints as defined in set_tms_timing_spec and set_tdi_timing_spec_* used for timing
    # analysis.
}
...
#    derive_clock_uncertainty
    ...
    # No matter where the device is in the chain.  The tck and tms are driven directly from JTAG hardware
    set_tck_timing_spec
    set_tms_timing_spec

    # Depending on where the device is located along the chain, tdi can be 
    # either driven by blaster hw (a. b.) or driven by another device in the chain(c. d.)
    # ---customize here---
    ...
    # Cut a few timing paths that are not related to JTAG logic in the FPGA core, such as security mode.
...
proc get_tck_delay_max { } {
    ...
    # tck delay on the PCB depends on the trace length from JTAG 10-pin header to FPGA on board.
...
proc set_tms_timing_spec { } {
    ...
    # tms delay on the PCB depends on the trace length from JTAG 10-pin header to FPGA on board.
    # In general on the PCB, the signal travels at the speed of ~160 ps/inch (1000 mils = 1 inch).
    # ---customize here---
...
proc set_tdi_timing_spec_when_driven_by_device { } {
    ...
    # tdi delay on the PCB depends on the trace length from JTAG 10-pin header to FPGA on board.
    # In general on the PCB, the signal travels at the speed of ~160 ps/inch (1000 mils = 1 inch).
    # ---customize here---
...
proc set_tdo_timing_spec_when_drive_blaster { } {
    ...
    #TDO does not latch inside the USB Blaster II at the rising edge of TCK, it actually is latched one half cycle later in packed mode (equivalent to 1 JTAG fall-to-fall cycles)
